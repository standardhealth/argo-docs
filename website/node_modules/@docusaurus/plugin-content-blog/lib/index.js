...
function pluginContentBlog(context, opts) {
    ...
    return {
        ...
        async contentLoaded({ content: blogContents, actions, }) {
            ...
            const blogItems = await Promise.all(blogPosts.map(async (blogPost) => {
                const { id, metadata } = blogPost;
                const { permalink } = metadata;
                const metadataPath = await createData(`${utils_1.docuHash(permalink)}.json`, JSON.stringify(metadata, null, 2));
                const temp = {
                    metadata,
                    metadataPath,
                };
